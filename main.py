...
@app.route('/<url>/<chr_underscore>/', methods=['GET', 'POST'])
def website(url="tempest", chr_underscore="none"):

    # Format the url correctly
    chr = chr_underscore.replace("_", " ");
    ...
    if(not url.lower() == new_url.lower()):
        return redirect("/" + new_url.lower() + "/", code=307);
    elif(not chr.lower() == new_chr.lower()):
        return redirect("/" + new_url.lower() + "/" + new_chr.lower().replace(" ", "_") + "/", code=307);
